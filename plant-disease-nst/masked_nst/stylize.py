...
def read_single_mask(path, hard_width): 
    ...
    # rawmask = cv2.imread(path).astype(np.float)
    if hard_width:
        rawmask = scipy.misc.imresize(rawmask, float(hard_width) / rawmask.shape[1], interp='nearest')    
    # actual_mask = rawmask > 0
    actual_mask = np.logical_and(np.logical_and(rawmask[:,:,0] > 0, rawmask[:,:,1] > 0), rawmask[:,:,2] > 0)
    rawmask[actual_mask] = 255
...
def read_colorful_mask(target_path, style_path, hard_width, n_colors):
    ...
    target_mask = cv2.imread(target_path).astype(np.float)
    
    style_mask = cv2.imread(style_path).astype(np.float)
...
def apply_output_mask(output_img, content_img_path, content_mask_path):
    rawmask = imageio.imread(content_mask_path)
    # rawmask = np.array([cv2.imread(content_mask_path).astype(np.float)])
    leaf_mask = np.logical_and(np.logical_and(rawmask[:,:,0] > 0, rawmask[:,:,1] > 0), rawmask[:,:,2] > 0)
    leaf_mask = np.tile(leaf_mask, (3,1,1,)).transpose(1,2,0)
    leaf_content = np.multiply(output_img[0], leaf_mask)
    # content_img = np.array([cv2.imread(content_img_path).astype(np.float)])
    content_img = imageio.imread(content_img_path)
    back_mask = ~leaf_mask
    back_content = np.multiply(content_img, back_mask)
    masked_output_img = np.array([leaf_content + back_content])
    return masked_output_img
...
def  main(args):
    ...
    result = sess.run(target_net['input'])
    # result = apply_output_mask(result, args.content_img, args.target_mask)
