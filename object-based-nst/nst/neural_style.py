...
STYLE_SCALE = 1.0
ITERATIONS = 1000
#https://github.com/anishathalye/neural-style?tab=readme-ov-file#requirements : Path for model.
VGG_PATH = "./nst/models/imagenet-vgg-verydeep-19.mat"
POOLING = 'max'
...
def main():
    ...
    if not os.path.isfile(options.network):
        urlretrieve('https://www.vlfeat.org/matconvnet/models/imagenet-vgg-verydeep-19.mat', './nst/models/yolov8x-seg.pt')
    ...
    model = YOLO("nst/models/yolov8x-seg.pt")
    content_image = imread(options.content)
    style_images = [imread(style) for style in options.styles]
    results = model(content_image)
    ...
    # Resizes all the style images using style_scale [FIXED VALUE 1] and then used interpolation to actually resize.
    for i in range(len(style_images)):
        ...
        size = tuple((np.array(im.size) * style_scale *
            target_shape[1] / style_images[i].shape[1]).astype(int))
    ...
    for result in results:
        for j, mask in enumerate(result.masks.data):
            ...
            masked_object = cv2.bitwise_and(content_image, content_image, mask=(mask).astype(np.uint8))
            ...
            # style_mask = cv2.bitwise_and(image, image, mask=(mask > 0).astype(np.uint8))
            # black_background = cv2.bitwise_not((mask > 0).astype(np.uint8))
            # black_background = black_background.astype(np.uint8)
            # original_region = cv2.bitwise_and(content_image, content_image, mask=black_background)
            # style_images = cv2.add(style_mask, original_region)
            # imsave(options.output, style_mask)
            imsave("./target.png",image)
            imsave("./mask.png",mask)
            imsave("./content.png",content_image)
            exit()
